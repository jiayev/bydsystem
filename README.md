...
### 更新事项
1.时间系统已完善，时间系统初始为步进模式，可以在客户端输入（5）让时间前进至指定时间


### To Do List
1. 时间系统(已完成)，可以设定开始时间(已完成)、时间单位(已完成)，可设置步进(已完成)或自动进行(已完成)，每个时间单位开始时进行检查和操作(已完成)，正在充电的车每次时间单位结束时计算费用，结束时加到详单中
2. 重写申请系统，首先将申请推至等待表中，然后检查对应充电桩的充电位和等待位是否有空，有空则将等待表中申请推至位置，直到没有空位或者等待表为空
3. 以上的检查需要在每个时间单位开始时进行
4. 退出充电桩功能，分为以下几个情况：（写了，但是？）
    1. 充电桩关闭，需要把充电位和等待位中的用户全部退出并返回等待表
    2. 用户主动退出或充电完成，如果用户在充电位，则将等待位中的用户推至充电位，如果用户在等待位，则直接移除；之后，重新检查
5. 账单与详单功能：（写了相关的逻辑，但能否正确执行？）
